...
async function s3Upload (req, res) {
    ...
    try{
        await uploadFileToS3(formData.file, 'file-project');
...
async function s3Get (req, res) {
    try{
        const bucketData = await getBucketListFromS3('file-project');
...
async function getSignedUrl(req, res) {
    try {
        ...
        const url = await getPresignedURL('file-project', key);
        res.send(url);
